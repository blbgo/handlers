...
func Setup(
	...
	r := &log{RouteParams: rp, RecordLog: recordLog, Renderer: rend}
...
func (r *log) log(rw http.ResponseWriter, req *http.Request) {
	...
	err := r.Range(recordlog.MinTime(), false, func(created time.Time, name string) bool {
...
func (r *log) view(rw http.ResponseWriter, req *http.Request) {
	...
	if len(offset) == 0 {
		if reverse {
			start = recordlog.MaxTime()
		} else {
			start = recordlog.MinTime()
		}
	} else {
